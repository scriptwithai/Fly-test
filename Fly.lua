...
local function updateFly()
    ...
    local finalVel = moveDir * speed

    -- Вертикальное управление (Space / Jump на мобильных)
    if UserInputService:IsKeyDown(Enum.KeyCode.Space) or UserInputService:IsKeyDown(Enum.KeyCode.E) then
        finalVel += Vector3.new(0, speed, 0)
    ...
    if linearVel then
        ...
        if alignOri then
            alignOri.CFrame = CFrame.new(root.Position, root.Position + camera.CFrame.LookVector)
        end
    end
end
